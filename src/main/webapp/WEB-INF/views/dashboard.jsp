...
<nav class="navbar navbar-expand-lg navbar-dark navbar-custom px-4 shadow-sm mb-4 sticky-top">
    ...
    <div class="card table-card">
        <div class="card-body p-0">
            <div class="p-4 bg-white border-bottom">
                <form action="${pageContext.request.contextPath}/account" method="get" class="input-group search-box" style="max-width: 400px;">
                    <input type="hidden" name="action" value="search">
                    <input type="text" name="keyword" class="form-control border" placeholder="Tìm kiếm..." value="${param.keyword}">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0" id="accountTable">
                    ...
                    <tr>
                        ...
                        <th class="text-end">Số dư / Tiền gửi</th>
                        <th>Chi tiết</th>
                        <th class="no-print text-center">Hành động</th>
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    <c:forEach var="acc" items="${listAccounts}" varStatus="status">
                        <tr>
                            ...
                            <td>
                                ...
                                <small class="text-muted">${acc.citizenId}</small>
                            </td>
                            <c:choose>
                                <c:when test="${acc.accountType == 'SavingsAccount'}">
                                    <td class="text-center"><span class="badge bg-success bg-opacity-10 text-success border border-success px-3">Tiết Kiệm</span></td>
                                    <td class="text-end fw-bold text-success"><fmt:formatNumber value="${acc.depositAmount}" pattern="#,###"/> đ</td>
                                    <td><small>Lãi: ${acc.interestRate}% | ${acc.term} tháng</small></td>
                                </c:when>
                                <c:otherwise>
                                    <td class="text-center"><span class="badge bg-primary bg-opacity-10 text-primary border border-primary px-3">Thanh Toán</span></td>
                                    <td class="text-end fw-bold text-primary"><fmt:formatNumber value="${acc.balance}" pattern="#,###"/> đ</td>
                                    <td><small>Thẻ: ${acc.cardNumber}</small></td>
                                </c:otherwise>
                            </c:choose>
                            <td class="text-center no-print">
                                <a href="account?action=edit&code=${acc.accountCode}" class="btn btn-light btn-sm text-primary"><i class="bi bi-pencil-square"></i></a>
                                <a href="account?action=delete&code=${acc.accountCode}" class="btn btn-light btn-sm text-danger" onclick="return confirm('Xóa tài khoản này?');"><i class="bi bi-trash"></i></a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
            ...
            <c:if test="${totalAccPages > 1}">
                <div class="p-3 border-top">
                    <ul class="pagination justify-content-center mb-0">
                        <li class="page-item ${currentAccPage == 1 ? 'disabled' : ''}">
                            <a class="page-link" href="account?page=${currentAccPage - 1}&logPage=${currentLogPage}">Trước</a>
                        </li>
                        <c:forEach begin="1" end="${totalAccPages}" var="i">
                            <li class="page-item ${currentAccPage == i ? 'active' : ''}">
                                <a class="page-link" href="account?page=${i}&logPage=${currentLogPage}">${i}</a>
                            </li>
                        </c:forEach>
                        <li class="page-item ${currentAccPage == totalAccPages ? 'disabled' : ''}">
                            <a class="page-link" href="account?page=${currentAccPage + 1}&logPage=${currentLogPage}">Sau</a>
                        </li>
                    </ul>
                </div>
            </c:if>
        ...
        <div class="card table-card mt-5 mb-5 border-danger">
            ...
            <div class="card-body p-0">
                <table class="table table-sm table-striped mb-0">
                    ...
                    <tr><th>Thời gian</th><th>Người dùng</th><th>Hành động</th></tr>
                    </thead>
                    <tbody>
